...
class store {
   ...
   * @version 180811 2.1 废弃了纠正后端错误的逻辑
   * @version 180912 2.2 服务器端切换到客户端时，源代码上state会遍历setState，导致没请求的key的_loaded为true，开放参数减少这个副作用
   * @param  {Any}     *data        值
   * @param  {String}  key          键值
   * @param  {Boolean} updateLoaded 是否开启调用后就在对象里面加入_loaded为true的作用
   * @return {Void}
   */
  @action
  setState(data, key = defaultKey, updateLoaded = true) {
    let prevState;
    let _data = data;
  ...
  async fetchListThenSetState(
    ...
    const _list = _refresh ? data[listKey] : [...list, ...data[listKey]];
